...
def crop_relative(image, top_ratio, bottom_ratio, left_ratio, right_ratio):
    """
    根据比例裁剪图像区域
...
def recognize(image,top,bot,left,right,config='',lang=None):

    if is_night_mode(np.array(image)):
    #黑夜模式
        gray=cv2.cvtColor(np.array(image),cv2.COLOR_BGR2GRAY)
        cropped_image=cv2.bitwise_not(gray)
        cropped_image=crop_relative(cropped_image,top,bot,left,right)
    else:
        #正常模式
        image_cv = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
        cropped_image=crop_relative(image_cv,top,bot,left,right)
...
def extract_info(text, keyword, pattern=None):
...
# 提取关键词之间的文本
def extract_between_keywords(text, start_keyword, end_keyword=None):
    if end_keyword:
...
def distill_from_bill(image):
    number_config = r'-c tessedit_char_whitelist=0123456789-. --psm 6'
    detail_config=r'--oem 2 --psm 6'
    chinese='chi_sim'
    text_amount=recognize(image,0.23,0.35,0.1,0.9,config=number_config)
    pay_to=recognize(image,0.2,0.25,0.1,0.9,lang=chinese)
    message=recognize(image,0.3,0.65,0.0,1.0,lang=chinese,config=detail_config)

    # print(message)

    #提取并且处理信息
    recipient = extract_info(pay_to, "扫二维码付款-给")
    amount_pattern = r'-\d+\.?\d*'
    amount = extract_info(text_amount, "-", amount_pattern)
    if amount and amount.startswith('-'):
        amount=amount[1:]
    status = extract_between_keywords(message, "当前状态", "收款方备注")
    remark = extract_between_keywords(message, "收款方备注", "付款方留言")
    note = extract_between_keywords(message, "付款方留言", "支付方式")
    payment_method = extract_between_keywords(message, "支付方式", "转账时间")
    transfer_time = extract_between_keywords(message, "转账时间", "转账单号")
    bill_id=extract_between_keywords(message,"转账单号")
